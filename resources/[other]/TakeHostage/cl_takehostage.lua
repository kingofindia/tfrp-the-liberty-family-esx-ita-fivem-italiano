...
RegisterCommand("ostaggio",function(source, args)
	...
	if not canTakeHostage then 
		drawNativeNotification("Hai bisogno di una pistola per prendere il player come ostaggio")
	end
	...
	if not holdingHostageInProgress and canTakeHostage then		
		...
		if closestPlayer ~= nil then
			...
			print("Nessun player nelle vicinanze")
			drawNativeNotification("Nessun player vicino da prendere come ostaggio")
		end 
	-- else
...
AddEventHandler('cmg3_animations:syncTarget', function(target, animationLib, animation2, distans, distans2, height, length,spin,controlFlag,animFlagTarget,attach)
	...
	if attach then 
		print("entità attaccata")
		AttachEntityToEntity(GetPlayerPed(-1), targetPed, 0, distans2, distans, height, 0.5, 0.5, spin, false, false, false, false, 2, false)
	else 
		print("entità non collegata")
	end
...
function GetClosestPlayer(radius)
	...
	print("il giocatore più vicino è distante: " .. tostring(closestDistance))
...
Citizen.CreateThread(function()
	while true do 
		if holdingHostage then
			...
			DrawText3D(playerCoords.x,playerCoords.y,playerCoords.z,"Premi [G] per rilasciarlo, [H] per ucciderlo")
