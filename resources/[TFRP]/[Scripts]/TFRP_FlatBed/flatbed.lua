...
AddTextEntry(controllerMessageLoweredCar, "Premi ~INPUT_CONTEXT~ per ~y~alzare ~w~il pianale.~n~Premi ~INPUT_DETONATE~ per ~r~staccare ~w~il veicolo.")
AddTextEntry(controllerMessageLoweredNoCar, "Premi ~INPUT_CONTEXT~ per ~y~alzare ~w~il pianale.~n~Premi ~INPUT_DETONATE~ per ~g~attaccare ~w~il veicolo.")
AddTextEntry(controllerMessageRaised, "Premi ~INPUT_CONTEXT~ per ~y~abbassare ~w~il pianale.")
...
function drawMarker(pos)
    ...
    if dist < 25.0 then
        DrawMarker(27, pos, vector3(0.0, 0.0, 0.0), vector3(0.0, 0.0, 0.0), vector3(1.0, 1.0, 1.0), 255, 255, 255, 150)
        if dist < 1.5 then
            return true
        end
    end
