...
local AMBULANCE2_HASH = Wrapper.GetHashKey('gresley')
--local AMBULANCE3_HASH = Wrapper.GetHashKey('f750')
...
function gatherData()
    ...
    if newPlayerData.vehicle ~= nil then
        ...
        elseif GetEntityModel(vehicle) == GetHashKey('gresley') then
            newPlayerData.isInAmbulance = Wrapper.IsVehicleModel(newPlayerData.vehicle, AMBULANCE2_HASH)
        --elseif GetEntityModel(vehicle) == GetHashKey('f750') then
            --newPlayerData.isInAmbulance = Wrapper.IsVehicleModel(newPlayerData.vehicle, AMBULANCE3_HASH)
        end
