        ...
        if HasScaleformMovieLoaded(scaleform) and UsingComputer then
            ...
            if GetScaleformMovieFunctionReturnBool(ClickReturn) then
                ...
                elseif Hacking and ProgramID == 86 then
                    ...
                    mHacking = false
                    TriggerServerEvent('esx_phone:send', 'police', 'Si è verificata una violazione dei dati ' .. v.BankName .. ' e il caveau della banca è stato aperto!', {x = v.Doors[#v.Doors].Coords.x, y = v.Doors[#v.Doors].Coords.y, z = v.Doors[#v.Doors].Coords.z})
                ...
                if Hacking then
                    ...
                    if lives <= 0 then
                        ...
                        Wait(5000)
                        local v = Config.BankRobbery[hackingBank]
                        TriggerServerEvent('esx_phone:send', 'police', 'Una persona anonima ha tentato di violare i ' .. v.BankName .. ' sistemi!', {x = v.Doors[#v.Doors].Coords.x, y = v.Doors[#v.Doors].Coords.y, z = v.Doors[#v.Doors].Coords.z})
...
function USBSuccess(success, timeremaining)
    if success then
        ...
        FreezeEntityPosition(PlayerPedId(), false)
        local v = Config.BankRobbery[hackingBank]
        ---TriggerServerEvent('qalle:jobS', 'police', 'Una persona anonima ha tentato di violare i ' .. v.BankName .. 's system!', {x = v.Doors[#v.Doors].Coords.x, y = v.Doors[#v.Doors].Coords.y, z = v.Doors[#v.Doors].Coords.z}, math.random(200, 1000))
        TriggerServerEvent('esx_phone:send', 'police', 'Una persona anonima ha tentato di violare i ' .. v.BankName .. ' sistemi!', {x = v.Doors[#v.Doors].Coords.x, y = v.Doors[#v.Doors].Coords.y, z = v.Doors[#v.Doors].Coords.z})
        
      ---  TriggerServerEvent('gcPhone:sendMessage2', 'police', 'Una persona anonima ha tentato di violare i ' .. v.BankName .. 's system!')
        blockKeys = false
        mHacking = false
    end
end
    ...
    while true do
        for k, v in pairs(Config.BankRobbery) do
            ...
            if GetDistanceBetweenCoords(coords, v.Hacking.Coords, true) <= 2.0 and v.Doors[#v.Doors].Frozen and not mHacking then
                ...
                if allowed then
                    TriggerEvent('loffe_heist:helpTimed', '~INPUT_CONTEXT~ scassina', 25)
                    if IsControlPressed(0, 38) then
                        ...
                        if police >= v.Cops then
                            ...
                            ESX.ShowNotification('Non c\'è abbastanza polizia online!')
                        end
                    end
                end
                Wait(10)
        ...
        for k, v in pairs(Config.BankRobbery) do
            ...
            if GetDistanceBetweenCoords(coords, v.Money.BoxPosition, true) <= 1.5 and not v.Doors[#v.Doors].Frozen then
                if v.Money.Amount > 0 and bag then
                    ...
                    AddTextComponentSubstringPlayerName('~INPUT_CONTEXT~ raccogli soldi')
                    ...
                    if IsControlPressed(0, 38) then
                        ...
                        if police >= v.Cops then
                            ...
                            while v.Money.Amount > 0 do
                                ...
                                TriggerEvent('loffe_heist:helpTimed', 'Premi ~INPUT_JUMP~ per smettere di raccogliere il denaro', 100)
                                Wait(350)
                ...
                if GetDistanceBetweenCoords(coords, d.Coords, true) <= 2.5 and d.Frozen then
                    if hasWeld then
                        while GetDistanceBetweenCoords(GetEntityCoords(player), d.Coords, true) <= 2.5 and d.Frozen and not d.Hacking do
                            ...
                            Wait(50)
                            if allowed then
                                TriggerEvent('loffe_heist:helpTimed', '~INPUT_CONTEXT~ scassina la porta', 50)
                                if IsControlPressed(0, 38) then
                                    local police = nil
                                    ESX.TriggerServerCallback('loffe_heist:getCops', function(cops)
                                        police = cops
                                    ...
                                    if police >= v.Cops then
                                        ...
                                        local welded = true
                                        for i = 1, 100 do
                                            TriggerEvent('loffe_heist:text', '[~b~SPACE~w~] ~r~smetti di scassinare', 0.955, 0.97, d.Time)
                                            drawSub('Scassinando ~b~' .. i .. '%', 4,1,0.5,0.96,0.6,255,255,255,255, d.Time)
                                            if IsDisabledControlPressed(0, 22) then
                                                ClearPedTasks(player)
                                                blockKeys = false
                                                welded = false
                ...
                if not s.Looted then
                    if hasDrill then
                        if GetDistanceBetweenCoords(coords, s.Coords, true) <= 1.0 and not Config.BankRobbery[i].Doors[#Config.BankRobbery[i].Doors].Frozen then
                            TriggerEvent('loffe_heist:helpTimed', '~INPUT_CONTEXT~ Perfora la cassaforte', 5)
                            if IsControlPressed(0, 38) then
                                local police = nil
                                ESX.TriggerServerCallback('loffe_heist:getCops', function(cops)
                                    police = cops
				...
				if not Config.BankRobbery[i].Doors[#Config.BankRobbery[i].Doors].Frozen then
                    if GetDistanceBetweenCoords(GetEntityCoords(PlayerPedId()), Config.BankRobbery[i].Doors[#Config.BankRobbery[i].Doors].Coords) <= 5 then
                        while GetDistanceBetweenCoords(GetEntityCoords(PlayerPedId()), Config.BankRobbery[i].Doors[#Config.BankRobbery[i].Doors].Coords) <= 5 do
                            TriggerEvent('loffe_heist:helpTimed', '~INPUT_CONTEXT~ Chiudi la porta', 25)
