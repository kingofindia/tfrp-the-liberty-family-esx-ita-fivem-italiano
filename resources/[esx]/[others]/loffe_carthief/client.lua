			...
			if GetDistanceBetweenCoords(coords, Config.dropoffPoints[dropoff].sx, Config.dropoffPoints[dropoff].sy, Config.dropoffPoints[dropoff].sz, true) < 50 then
				...
				repeat
					...
					if IsPedInVehicle(GetPlayerPed(-1), spawned_cars, false) then
						TriggerServerEvent('esx_addons_gcphone:startCall', 'police', (_U('police_message')), true, {x = Config.dropoffPoints[dropoff].sx, y = Config.dropoffPoints[dropoff].sy, z = Config.dropoffPoints[dropoff].sz}, true)
						ClearPrints()
						SetTextEntry_2("STRING")
...
Citizen.CreateThread(function()
	for k,v in pairs(Config.Zones) do
  	for i = 1, #v.Pos, 1 do
		...
		AddTextComponentString('Ladro d\'auto')
