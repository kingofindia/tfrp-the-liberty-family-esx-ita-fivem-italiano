...
AddEventHandler('jsfour-atm:insert', function(amount)
    ...
    TriggerClientEvent("esx_jsfouratm:showNotification", _source, 'Importo non valido')
	else
		xPlayer.removeMoney(amount)
		xPlayer.addAccountMoney('bank', amount)
    --TriggerClientEvent('esx:showNotification', _source, 'Hai depositato $' .. amount .. '~s~')
    TriggerClientEvent("esx_jsfouratm:showNotification", _source, 'Hai depositato $' .. amount .. '~s~')
	end
end)
...
AddEventHandler('jsfour-atm:take', function(amount)
    ...
    --TriggerClientEvent('esx:showNotification', _source, 'Hai prelevato $' .. amount .. '~s~ ')
    TriggerClientEvent("esx_jsfouratm:showNotification", _source, 'Hai prelevato $' .. amount .. '~s~ ')
  ...
  function (result)
    if (result[1] ~= nil) then
      ...
      local senPlayer    = ESX.GetPlayerFromId(_source)
    	...
    	if amount >= accountMoney then
        print("JSFOUR-ATM: ERROR")
        TriggerClientEvent("esx_jsfouratm:showNotification", _source, 'Transazione impossibile')
    	else
        ...
        function (result)
          if (result[1] ~= nil) then
            --TriggerClientEvent('esx:showNotification', _source, 'Hai inviato $' .. amount .. '~s~ a ' .. result[1].firstname .. ' ' .. result[1].lastname)
            TriggerClientEvent("esx_jsfouratm:showNotification", _source, 'Hai inviato $' .. amount .. '~s~ a ' .. result[1].firstname .. ' ' .. result[1].lastname)
            --TriggerClientEvent('esx:showNotification', recPlayer.source, 'Hai ricevuto $' .. amount .. '~s~ da ' .. result[1].firstname .. ' ' .. result[1].lastname)
            TriggerClientEvent("esx_jsfouratm:showNotification", recPlayer.source, 'Hai ricevuto $' .. amount .. '~s~ da ' .. result[1].firstname .. ' ' .. result[1].lastname)
          end
        end)
      end
    end
