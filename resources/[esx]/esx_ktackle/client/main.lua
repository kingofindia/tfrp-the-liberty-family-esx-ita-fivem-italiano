...
local FbiJob 				    = 'state'
local GiudiceJob 				= 'maluma'
local SceriffoJob 				= 'sceriffo'
...
Citizen.CreateThread(function()
	while true do
		...
		if IsControlPressed(0, Keys['G']) and not isTackling and GetGameTimer() - lastTackleTime > 10 * 1000 and (PlayerData.job.name == PoliceJob or PlayerData.job.name == FbiJob or PlayerData.job.name == GiudiceJob or PlayerData.job.name == SceriffoJob)  then
			Citizen.Wait(10)
