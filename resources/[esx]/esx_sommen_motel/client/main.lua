...
Citizen.CreateThread(function()
	...
	CacheApartments(function()
		for k,v in pairs(Apartments) do
			...
			local enterMessage = 'Premi ~INPUT_CONTEXT~ per entrare nella stanza'

			if not owns then
				enterMessage = 'Premi ~INPUT_CONTEXT~ per entrare nella stanza'
			end
			...
			if owned then
				...
				Markers.AddMarker('apartment_storage', v.storage, 'Premi ~INPUT_CONTEXT~ per aprire il menu inventario', function()
			...
			if owned or invited then
				...
				Markers.AddMarker('apartment_closet', v.closet, 'Premi ~INPUT_CONTEXT~ per cambiare i vestiti', function()
		    ...
		    Markers.AddMarker('apartment_exit', v.exit, 'Premi ~INPUT_CONTEXT~ per aprire il menu', function()
				local elements = {
					{label = 'Esci', value = 'exit'}
				}

				if owned or invite then
					table.insert(elements, {label = 'Invita un amico', value = 'invite'})
				end
...
function OpenStorageMainMenu(apartment)
	local elements = {
		...
		--{label = 'Denaro sporco', value = 'safe'},
		{label = 'Droga', value = 'drugs'}
	}
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'apartment_storage_option',
		{
			...
			align = 'top-left',
			elements = {
				{label = 'Prendi oggetto', value = 'get'},
				{label = 'Deposita oggetto', value = 'put'}
			}
		},
		function(data, menu)
			menu.close()
							...
							if v ~= nil and v.count ~= nil then
								if v.count > 0 then
									if v.money == true then
										table.insert(elements, {label = 'Soldi sporchi (' .. v.count .. ' $)', value = k, amount = v.count, rawLabel = 'Dirty Money'})								
									else
						...
						ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'apartment_storage_items',
							...
							function(data2, menu)
				        		...
				        		OpenQuantityMenu(function(count)
				        			if count <= 0 then
				        				ESX.ShowNotification('~r~Valore non valido.')
				        			...
				        			if items[storage] ~= nil and items[storage][itemName] ~= nil then	    
				        				if items[storage][itemName].weapon ~= true then
					        				if items[storage][itemName].count >= count then
							        			...
							        			ESX.ShowNotification('Non hai ~r~x' .. count .. ' ' .. data2.current.rawLabel)
							        		end
							        	end
				        			else
				        				ESX.ShowNotification('Non hai ~r~x' .. count .. ' ' .. data2.current.rawLabel)
					...
					ESX.TriggerServerCallback('esx_sommen_motel:getInventory', function(inventory)
				    	ESX.TriggerServerCallback('esx_sommen_motel:getDirtyMoney', function(money)
					    	...
					    	if storage == 'safe' then
					    		table.insert(elements, {label = 'Soldi sporchi (' .. money .. ' $)', value = 'dirty', amount = money, money = true})	
					    	else
					        			...
					        			OpenQuantityMenu(function(count)
					        				...
					        				if money >= count then
					        					if items[storage] ~= nil and items[storage][itemName] ~= nil then
							        				...
							        				items[storage][itemName] = {count = count, label = 'Soldi sporchi', money = true}
							        			end
					        					...
					        					ESX.ShowNotification("~r~ Azione impossibile")
					        				end
					        			end)
...
function OpenApartmentMenu(apartment, owned)
	local values = GetApartmentValues(apartment)
		...
		if owned then
			...
			Markers.AddMarker('apartment_' .. apartment, values.enter, 'Premi ~INPUT_CONTEXT~ per entrare nella stanza', function()
				OpenApartmentMenu(apartment, true)
			end)	

			SetEntityCoords(GetPlayerPed(-1), 151.38, -1007.95, -99.0 - 1.0)

			Notifications.PlaySpecialNotification("Benvenuto nel Motel")				
...
function OpenConfirmationMenu(callback)
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'confirmation_menu',
		{
			title = 'Sei stato invitato vuoi entrare?',
			align = 'top-left',
			elements = {
				{label = 'Si', value = 'yes'},
				{label = 'No', value = 'no'}
    ...
    while true do
        ...
        Citizen.Wait(5)
            ...
            if distance < 3 and InAction == false then
                Drawing.draw3DText(154.2, -1006.01, -101.0, 'Premi ~g~[E] ~w~per riposarti', 6, 0.07, 0.07, 255, 255, 255, 215)
    ...
    Citizen.CreateThread(function()
        while true do
            ...
            if InAction == true then
                headsUp('Premi ~INPUT_VEH_DUCK~ per alzarti')
    ...
    while true do
        ...
        Citizen.Wait(5)
            ...
            if distance < 2.3 and InAction == false then
                Drawing.draw3DText(154.64, -1001.16, -101.0, 'Premi ~g~[E] ~w~per utilizzare la toilette', 6, 0.07, 0.07, 255, 255, 255, 215)
              if distance < 1 and InAction == false then
                if IsControlJustReleased(0, Keys['E']) then
