...
function ParkBoats()
	MySQL.Async.execute('UPDATE owned_boats SET stored = true WHERE stored = @stored AND vehicleType = @vehicleType',
	{
		['@stored']      = false,
		['@vehicleType'] = 'boat'
...
ESX.RegisterServerCallback('esx_boat:buyBoat', function (source, cb, vehicleProps)
	...
	if xPlayer.getMoney() >= price then
		...
		MySQL.Async.execute('INSERT INTO owned_boats (owner, plate, vehicle, vehicleType) VALUES (@owner, @plate, @vehicle, @vehicleType)',
		{
...
ESX.RegisterServerCallback('esx_boat:getGarage', function (source, cb)
	MySQL.Async.fetchAll('SELECT * FROM owned_boats WHERE owner = @owner AND vehicleType = @vehicleType AND stored = @stored',
	{
		['@owner']       = GetPlayerIdentifiers(source)[1],
		['@vehicleType'] = 'boat',
		['@stored']      = true
