...
function OpenShopCenter()
	...
	ESX.UI.Menu.CloseAll()
  	...
  	local elements = {}
		...
		if showblip then
			table.insert(elements, {label = 'Nascondi tutti i negozi dalla mappa', value = 'removeblip'})
		else
			table.insert(elements, {label = 'Mostra tutti i negozi in mappa', value = 'showblip'})
		end
...
function OpenBoss()
  ...
  ESX.TriggerServerCallback('esx_kr_shop:getOwnedShop', function(data)
	...
	local elements = {}
		...
		table.insert(elements, {label = 'Hai: $' .. data[1].money .. ' nel tuo negozio',    value = ''})
		table.insert(elements, {label = 'Spedizioni',    value = 'shipments'})
        table.insert(elements, {label = 'Metti un oggetto in vendita', value = 'putitem'})
        table.insert(elements, {label = 'Prendi un oggetto in vendita',    value = 'takeitem'})
        table.insert(elements, {label = 'Metti i soldi nel tuo negozio',    value = 'putmoney'})
        table.insert(elements, {label = 'Prendi i soldi dal tuo negozio',    value = 'takemoney'})
        table.insert(elements, {label = 'Cambia il nome del negozio: $' .. Config.ChangeNamePrice,    value = 'changename'})
		table.insert(elements, {label = 'Vendi il negozio al prezzo di $' .. math.floor(data[1].ShopValue / Config.SellValue),   value = 'sell'})
		...
		elseif data.current.value == 'sell' then
		  ...
		  ESX.UI.Menu.Open('dialog', GetCurrentResourceName(), 'sell', {
			title = 'SCRIVI: (YES) senza parentesi per confermare'
...
function OpenShipments(id)
	...
	table.insert(elements, {label = 'Ordina merce', value = 'buy'})
	table.insert(elements, {label = 'Spedizione', value = 'shipments'})
...
function GetAllShipments(id)
	...
	local once2 = true
		...
		for i=1, #items, 1 do
			...
			table.insert(elements, {label = '--SPEDIZIONE PRONTA--'})
			table.insert(elements, {label = 'Ricevi tutte le spedizioni', value = 'pickup'})
			once2 = false
			end
			...
			if time - items[i].time <= Config.DeliveryTime and once then
				table.insert(elements, {label = '--SPEDIZIONE IN CORSO--'})
				once = false
			end

			if time - items[i].time <= Config.DeliveryTime then
				times = time - items[i].time
				table.insert(elements, {label = items[i].label .. ' tempo rimasto: ' .. math.floor((Config.DeliveryTime - times) / 60) .. ' minuti' })
			end
...
function OpenShipmentDelivery(id)
	...
	local elements = {}
		...
		for i=1, #Config.Items, 1 do
			table.insert(elements, {labels =  Config.Items[i].label, label =  Config.Items[i].label .. ' prezzo $' .. Config.Items[i].price .. '  ',	value = Config.Items[i].item, price = Config.Items[i].price})
		end
...
function TakeItem(number)
  ...
  ESX.TriggerServerCallback('esx_kr_shop:getShopItems', function(result)
	...
	for i=1, #result, 1 do
	    if result[i].count > 0 then
	    	table.insert(elements, {label = result[i].label .. ' | ' .. result[i].count ..' pezzi in magazzino [' .. result[i].price .. ' $ l\'uno', value = 'removeitem', ItemName = result[i].item})
	    end
    end
    ...
    for i=1, #result.items, 1 do
      ...
      local invitem = result.items[i]
	    ...
	    if invitem.count > 0 then
			table.insert(elements, { label = invitem.label .. ' | ' .. invitem.count .. ' nel tuo inventario', count = invitem.count, name = invitem.name})
	    end
	end
  ...
  function(data, menu)
        ...
        local invcount = data.current.count
			...
			if count > invcount then
				ESX.ShowNotification('~r~Non puoi vendere più di quanto possiedi!')
				menu2.close()
				menu.close()
			else
				menu2.close()
...
Citizen.CreateThread(function()
	Citizen.Wait(500)
		...
		AddTextComponentString('Acquisto Negozio')
			...
			if result.cb ~= nil then
                ...
                TriggerServerEvent('esx_kr_shops-robbery:UpdateCanRob', id)
                    ...
                    local coords = {
						...
						TriggerServerEvent('esx_phone:send', "police", "RAPINA IN CORSO " .. result.name .. '\'s NEGOZIO', true, coords)
						TriggerServerEvent('esx_kr_shops-robbery:NotifyOwner', "~r~IL TUO NEGOZIO ~b~(" .. result.name .. ')~r~ STA PER ESSERE RAPINATO', id)
					...
					ESX.ShowNotification("~r~Non ci sono abbastanza poliziotti online " .. results .. '/' .. Config.RequiredPolices)
				end
			else
				ESX.ShowNotification("~r~Questo negozio è stato già rapinato, attendi " ..  math.floor((Config.TimeBetweenRobberies - result.time)  / 60) .. ' minuti')
			end
		end)
	end, id)
end
        ...
        Wait(0)
			local playerpos = GetEntityCoords(GetPlayerPed(-1))
				if OnRobbery and GetDistanceBetweenCoords(playerpos.x, playerpos.y, playerpos.z, Coordss.x, Coordss.y, Coordss.z, true) <= 15 then
					...
					TriggerEvent("mt:missiontext", "DANNEGGIAMENTO CASSAFORTE:~r~ " .. hp/100 .. "%", 1000)

				if hp == 0 then
					OnRobbery = false
					TriggerServerEvent('esx_kr_shops-robbery:GetReward', Id)
					TriggerServerEvent("esx_kr_shops-robbery:NotifyOwner", '~r~La rapina al tuo negozio ~b~(' .. Name ..')~r~ è avvenuta con successo!', Id)
					DeleteEntity(Var)
				end
				...
				elseif OnRobbery and GetDistanceBetweenCoords(playerpos.x, playerpos.y, playerpos.z, Coordss.x, Coordss.y, Coordss.z, true) >= 15 then
					OnRobbery = false
					DeleteEntity(Var)
					TriggerServerEvent('esx_kr_shops-robbery:NotifyOwner', "~g~La rapina al tuo negozio ~b~(" .. Name .. ')~g~ � stata sventata', Id)
