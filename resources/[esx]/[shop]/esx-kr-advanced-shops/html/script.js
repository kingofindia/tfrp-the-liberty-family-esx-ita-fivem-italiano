...
$(function() {
		window.addEventListener('message', function(event) {
			...
			for (i = 0; i < event.data.result.length; i++) {
				...
				$('#wrapper').append(
					`<div class = "image" id = ${event.data.result[i].item} label = ${event.data.result[i].label} count = ${event.data.result[i].count} price = ${event.data.result[i].price}>
						...
						<h5 class = "h4">In magazzino: ${event.data.result[i].count}x</h5>
					</div>`
				);
			}
			...
			if (event.data.owner == true) {
				...
				$('#wrapper').append(
					`
					<button class = "button" id = "bossactions" style = "position: absolute; right: 15px; top: 5px;">Menù Direttore</button>
					`
				);
			}
				...
				var i;
					for (i = 0; i < cb.length; i++) { 
						...
						$('#cart').append(
							...
							<h6>In magazzino: ${cb[i].count}</h4>
							<input type="text" id = ${cb[i].item} count = ${cb[i].count} class = "textareas" placeholder = "Quantità?"></textarea>
							</div>`
								);
							};
							...
							$('#cart').append(
							`
							<button class = "button" id = "buybutton" style = "position: absolute; right: 15px; top: 5px;">Acquista</button>
							<button class = "button" id = "back" style = "position: absolute; left: 15px; top: 5px;">Indietro</button>
							`
						);
					});
			});
			...
			$("body").on("click", "#buybutton", function() {
				...
				for (i = 0; i < value.length; i++) {
					...
					if (parseInt(count) >= parseInt(value[i].value) && parseInt(value[i].value) != 0 && isNumber) {
						...
						$.post('http://esx-kr-advanced-shops/notify', JSON.stringify({msg : "~r~Non valido."}));
					}
				}
			});
