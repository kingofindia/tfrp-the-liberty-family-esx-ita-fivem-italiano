  ...
  local identifier =  ESX.GetPlayerFromId(src).identifier
        ...
        function(data)
            ...
            if count > data[1].count then
                ...
                TriggerClientEvent('esx:showNotification', xPlayer.source, '~r~Non puoi prendere più di quanto possiedi')
                else
      ...
      MySQL.Async.fetchAll(
        ...
        if data[1] == nil then
            ...
            elseif data ~= nil and data[1].price ~= Price then
                Wait(250)
                TriggerClientEvent('esx:showNotification', xPlayer.source, '~r~Hai già uno stesso oggetto nel tuo negozio, ~r~ma al prezzo di ' .. data[1].price .. '. Inserisci il prezzo ' .. Price)
                Wait(250)
                TriggerClientEvent('esx:showNotification', xPlayer.source, '~r~Rimuovi l\'oggetto ed inserisci un nuovo prezzo o inserisci lo stesso prezzo')
            end
        end)  
    end)
end)
  ...
  local xPlayer = ESX.GetPlayerFromId(src)
        ...
        'SELECT * FROM owned_shops WHERE ShopNumber = @Number',
        {
            ...
            if xPlayer.getMoney() < ItemCount * result[1].price then
                TriggerClientEvent('esx:showNotification', src, '~r~Non hai abbastanza soldi.')
            elseif ItemCount <= 0 then
                TriggerClientEvent('esx:showNotification', src, '~r~Valore non valido.')
            else
                xPlayer.removeMoney(ItemCount * result[1].price)
                TriggerClientEvent('esx:showNotification', xPlayer.source, '~g~Hai acquistato ' .. ItemCount .. 'x ' .. Item .. ' al prezzo di $' .. ItemCount * result[1].price)
  ...
  local identifier = ESX.GetPlayerFromId(_source).identifier
    ...
    MySQL.Async.fetchAll('SELECT money FROM owned_shops WHERE ShopNumber = @ShopNumber AND identifier = @identifier',{['@ShopNumber'] = id,['@identifier'] = identifier,}, function(result)
        ...
        if result[1].money >= price * count then
            ...
            TriggerClientEvent('esx:showNotification', _source, '~g~Hai ordinato ' .. count .. ' ' .. label .. ' al prezzo di $' .. price * count)
        else
            TriggerClientEvent('esx:showNotification', _source, '~r~Non hai abbastanza soldi nel tuo negozio.')
        end
    end)
end)
...
local identifier = ESX.GetPlayerFromId(source).identifier
    ...
    if result[1].identifier == '0' then
        ...
        if xPlayer.getMoney() >= price then
            ...
            TriggerClientEvent('esx:showNotification', _source, '~g~Hai acquistato il negozio al prezzo di $' ..  price)
        else    
            TriggerClientEvent('esx:showNotification', _source, '~r~Non puoi permetterti questo negozio')
        end
...
local identifier = ESX.GetPlayerFromId(source).identifier
    ...
    MySQL.Async.fetchAll(
        ...
        if os.time() - result[1].LastRobbery <= 900 then
            ...
            TriggerClientEvent('esx:showNotification', xPlayer.source, '~r~Il denaro del tuo negozio è stato bloccato a causa di una rapina! Attendi ' .. math.floor((900 - time) / 60) .. ' minuti')
            return
        end
        ...
        TriggerClientEvent('esx:showNotification', xPlayer.source, '~g~Hai inserito $' .. amount .. ' nel tuo negozio')
        else
        TriggerClientEvent('esx:showNotification', xPlayer.source, '~r~Non puoi inserire più di quanto possiedi')
        end
    end)
end)
  ...
  MySQL.Async.fetchAll(
    ...
    if os.time() - result[1].LastRobbery <= 900 then
        ...
        if result[1].money >= amount then
            ...
            TriggerClientEvent('esx:showNotification', xPlayer.source, '~g~Hai prelevato $' .. amount .. ' dal tuo negozio')
    ...
    function(result)
      ...
      if result[1].money == 0 and result2[1] == nil then
        MySQL.Async.fetchAll("UPDATE owned_shops SET identifier = @identifiers, ShopName = @ShopName WHERE identifier = @identifier AND ShopNumber = @Number",
        {
            ...
            TriggerClientEvent('esx_kr_shops:removeBlip', -1)
            TriggerClientEvent('esx_kr_shops:setBlip', -1)
            TriggerClientEvent('esx:showNotification', xPlayer.source, '~g~Hai venduto il negozio')
            else
            TriggerClientEvent('esx:showNotification', xPlayer.source, '~r~Non puoi vendere il tuo negozio con oggetti o denaro al suo interno')
            end
        end)
    end)
end)
