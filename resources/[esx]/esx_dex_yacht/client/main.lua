    ...
    while true do
        ...
        local ped = PlayerPedId()
            local yacht = AddBlipForCoord(-2084.88, -1018.12, 12.78)
                ...
                AddTextComponentString("Rapina allo yacht")
        ...
        if GetDistanceBetweenCoords(GetEntityCoords(ped), -2084.88, -1018.12, 12.78, true) < 50 then
            DrawMarker(35, -2084.95, -1018.08, 12.78, 0, 0, 0, 0, 0, 75.0, 1.0, 1.0, 1.0, 0, 155, 253, 155, 1, 0, 2, 0, 0, 0, 0)
                if GetDistanceBetweenCoords(GetEntityCoords(ped), -2084.88, -1018.12, 12.78, true) < 1 then
                    ESX.ShowHelpNotification("Premi ~INPUT_CONTEXT~ per iniziare la rapina.")
      ...
      Citizen.Wait(1)
        ...
        if location1 == false and tags == true then
            ...
            if GetDistanceBetweenCoords(GetEntityCoords(ped), -2054.18, -1032.38, 8.97, true) < 1 then
                ESX.ShowHelpNotification("Premi ~INPUT_CONTEXT~ per cercare.")
                    if IsControlJustReleased(1, 51) then
                        Animacja()
                        location1 = true
                        searched = searched+1
    ...
    while searched < 20 do 
        Citizen.Wait(100)
            if searched == 20 then
                ...
                tags = false
                Citizen.Wait(19000)
                ESX.ShowHelpNotification("È stata trovata una valigia con soldi! Scappa dallo yacht per porre fine alla rapina.")
            end
        end
    end)
...
AddEventHandler("esx_yacht:notify", function(text)
    ESX.ShowNotification("La rapina sullo yacht è iniziata. Non lasciare che la valigia con i soldi venga rubata.")
