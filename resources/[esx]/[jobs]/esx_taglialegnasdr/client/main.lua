					...
					if (GetDistanceBetweenCoords(pos, -534.32, 5373.79, 69.50) < 2.75) then
						...
						if IsControlJustReleased(1, 51) then
							ESX.TriggerServerCallback('esx_ambulancejob:getItemAmount', function(quantity)
								...
								if quantity <= 1000 and futuroinventario <= 1000 then
							iniziato = true
						        ...
						        ESX.TriggerServerCallback('esx_ambulancejob:getItemAmount', function(quantity)
									if quantity >= 100 then
										ESX.TriggerServerCallback('esx_ambulancejob:getItemAmount', function(quantity)
											...
											if futuroinventario2 <= 1000 then
											  --statagliando = true
								...
								ESX.TriggerServerCallback('esx_ambulancejob:getItemAmount', function(quantity)
									if quantity >= 100 then
										ESX.TriggerServerCallback('esx_ambulancejob:getItemAmount', function(quantity)
											...
											if futuroinventario3 <= 1000 then
												--Wait(1000)
										        FreezeEntityPosition(playerPed, true)
										        --assi = true
...
Citizen.CreateThread(function()
	while true do
		...
		local pos = GetEntityCoords(playerPed, true)
		if PlayerData.job ~= nil and PlayerData.job.name == 'lumberjack' then
			if (GetDistanceBetweenCoords(pos, 1201.35, -1327.51, 34.22) < 15.0) and not iniziato and not invendita and inservizio then
				DrawMarker(29, 1201.35, -1327.51, 35.22, 0, 0, 0, 0, 0, 0, 1.5, 1.5, 1.5, 0, 255, 0, 150, 0, true, 2, 0, 0, 0, 0)
				if (GetDistanceBetweenCoords(pos, 1201.35, -1327.51, 34.22) < 0.75) then
					drawTxt('~b~Premi ~g~E~b~ per vendere le tavole di legno')
					if IsControlJustReleased(0, Keys['E']) then
						invendita = true
						local cVeh = GetVehiclePedIsUsing(playerPed, false)
					    local vehicle = GetHashKey('MULE')
					    if GetEntityModel(cVeh) == vehicle then
							ESX.TriggerServerCallback('esx_ambulancejob:getItemAmount', function(quantity)
								if quantity >= 100 then
									...
									--FreezeEntityPosition(playerPed, true)
									Citizen.Wait(10000)
									TriggerServerEvent('esx_taglialegnasdr:consegna')
									Wait(5000)
									invendita = false
									--FreezeEntityPosition(playerPed, false)
								else
