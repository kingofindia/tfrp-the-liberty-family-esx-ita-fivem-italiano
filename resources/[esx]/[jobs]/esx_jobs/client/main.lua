				...
				TriggerEvent('skinchanger:getSkin', function(skin)
					...
					if skin.sex == 0 then
					  ...
					  local clothesSkin = {
						...
						['decals_1'] = 0, ['decals_2'] = 0,
						['arms'] = 0,
						['pants_1'] = 75, ['pants_2'] = 6,
						['shoes_1'] = 52, ['shoes_2'] = 0,
						['ears_1'] = 8, ['ears_2'] = 2,
						['glasses_1'] = 15, ['glasses_2'] = 3
					  }
					  TriggerEvent('skinchanger:loadClothes', skin, clothesSkin)
					end
				end)
		...
		if (PlayerData.job ~= nil and PlayerData.job.name ~= 'unemployed') then
			...
			if zones ~= nil then
				...
				if IsControlJustReleased(0, Keys['E']) and not menuIsShowed and isInMarker then
					if onDuty or zone.Type == "cloakroom" or PlayerData.job.name == "reporters" then
				...
				if isInMarker and not menuIsShowed then
					hintIsShowed = true
					if (onDuty or zone.Type == "cloakroom" or PlayerData.job.name == "reporters") and zone.Type ~= "vehdelete" then
						hintToDisplay = zone.Hint
						hintIsShowed = true
					elseif zone.Type == "vehdelete" and (onDuty or PlayerData.job.name == "reporters") then
						local playerPed = PlayerPedId()
