...
ESX.RegisterServerCallback('esx_vehicleshop:retrieveJobVehiclesPolice', function (source, cb, type)
	local xPlayer = ESX.GetPlayerFromId(source)

	MySQL.Async.fetchAll('SELECT * FROM owned_police WHERE type = @type AND job = @job', {
		['@type'] = type,
		['@job'] = xPlayer.job.name
	}, function (result)
		cb(result)
...
RegisterServerEvent('esx_vehicleshop:setJobVehicleStatePolice')
AddEventHandler('esx_vehicleshop:setJobVehicleStatePolice', function(plate, state)
	local xPlayer = ESX.GetPlayerFromId(source)

	MySQL.Async.execute('UPDATE owned_police SET `stored` = @stored WHERE plate = @plate AND job = @job', {
		['@stored'] = state,
		['@plate'] = plate,
		['@job'] = xPlayer.job.name
	}, function(rowsChanged)
